...
function getHw(id: number): Hardware {
    let data = null;
    ...
    if (!data) throw new Error("dil nenalezen.");
...
function getSw(id: number, includeOffice: boolean = false, includeFlash: boolean = false): Software {
    let data = null;
    ...
    // pokud jsme nic nenasli, vyhodime chybu
    if (!data) throw new Error("softver nenalezen.");
...
function printSetup(name: string, setup: ConfigItem[]) {
    console.log(`\n--- ${name} ---`);
    let total = 0;
